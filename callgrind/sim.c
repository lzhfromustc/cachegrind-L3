...
static void cachesim_initcache(cache_t config, cache_t2* c)
{
   ...
   c->tags =
      (UWord*)CLG_MALLOC("cl.sim.cs_ic.1", sizeof(UWord) * c->sets * c->assoc);
...
static void cacheuse_initcache(cache_t2* c)
{
   ...
   c->use = CLG_MALLOC("cl.sim.cu_ic.1", sizeof(line_use) * c->sets * c->assoc);
   c->loaded =
      CLG_MALLOC("cl.sim.cu_ic.2", sizeof(line_loaded) * c->sets * c->assoc);
   c->line_start_mask =
      CLG_MALLOC("cl.sim.cu_ic.3", sizeof(int) * c->line_size);
   c->line_end_mask = CLG_MALLOC("cl.sim.cu_ic.4", sizeof(int) * c->line_size);
...
#define UPDATE_USE(L)                                                          \
   ...
   {                                                                           \
      ...
      CLG_DEBUG(2,                                                             \
                ...
                cache->name, idx, CLG_(bb_base) + current_ii->instr_offset,    \
                memline, mask);                                                \
      if (use->count > 0) {                                                    \
         CLG_DEBUG(                                                            \
...
static void log_1I0D(InstrInfo* ii)
{
   ...
   current_ii = ii;
   IrRes =
      (*simulator.I1_Read)(CLG_(bb_base) + ii->instr_offset, ii->instr_size);
   ...
   if (CLG_(current_state).collect) {
      ...
      if (CLG_(current_state).nonskipped)
         ...
         cost_Ir =
            CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_IR];

      inc_costs(IrRes, cost_Ir, CLG_(current_state).cost + fullOffset(EG_IR));
   }
}
...
static void log_2I0D(InstrInfo* ii1, InstrInfo* ii2)
{
   ...
   current_ii = ii1;
   Ir1Res =
      (*simulator.I1_Read)(CLG_(bb_base) + ii1->instr_offset, ii1->instr_size);
   current_ii = ii2;
   Ir2Res =
      (*simulator.I1_Read)(CLG_(bb_base) + ii2->instr_offset, ii2->instr_size);

   CLG_DEBUG(6, "log_2I0D:  Ir1 %#lx/%u => %s, Ir2 %#lx/%u => %s\n",
             CLG_(bb_base) + ii1->instr_offset, ii1->instr_size,
             cacheRes(Ir1Res), CLG_(bb_base) + ii2->instr_offset,
             ii2->instr_size, cacheRes(Ir2Res));
...
static void log_3I0D(InstrInfo* ii1, InstrInfo* ii2, InstrInfo* ii3)
{
   ...
   current_ii = ii3;
   Ir3Res =
      (*simulator.I1_Read)(CLG_(bb_base) + ii3->instr_offset, ii3->instr_size);
   ...
   if (CLG_(current_state).collect) {
      ...
      if (CLG_(current_state).nonskipped) {
         ...
         cost_Ir =
            CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_IR];
         cost_Dr =
            CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_DR];
      }

      inc_costs(IrRes, cost_Ir, CLG_(current_state).cost + fullOffset(EG_IR));
      inc_costs(DrRes, cost_Dr, CLG_(current_state).cost + fullOffset(EG_DR));
   }
}
   ...
   if (CLG_(current_state).collect) {
      ...
      if (CLG_(current_state).nonskipped) {
         ...
         cost_Ir =
            CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_IR];
         cost_Dw =
            CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_DW];
      }

      inc_costs(IrRes, cost_Ir, CLG_(current_state).cost + fullOffset(EG_IR));
      inc_costs(DwRes, cost_Dw, CLG_(current_state).cost + fullOffset(EG_DW));
   }
}
...
static void cachesim_post_clo_init(void)
{
   ...
   // TODO: I changed cachegrind to have 3 levels of cache which impacts here
   // Should remove the first LLc below and replace with L2
   /* Configuration of caches only needed with real cache simulation */
   VG_(post_clo_init_configure_caches)(&I1c, &D1c, &LLc, &LLc, &clo_I1_cache,
...
static void cachesim_printstat(Int l1, Int l2, Int l3)
{
   ...
   CLG_(copy_cost)(CLG_(get_event_set)(EG_DR), D_total,
                   total + fullOffset(EG_DR));
   CLG_(add_cost)(CLG_(get_event_set)(EG_DW), D_total,
...
void CLG_(init_eventsets)(void)
{
   ...
   if (clo_collect_cacheuse)
      CLG_(register_event_group4)(EG_USE, "AcCost1", "SpLoss1", "AcCost2",
                                  "SpLoss2");
   ...
   if (CLG_(clo).collect_systime != systime_no) {
      if (CLG_(clo).collect_systime == systime_nsec)
         CLG_(register_event_group3)(EG_SYS, "sysCount", "sysTime",
                                     "sysCpuTime");
      else
...
static void
cachesim_add_icost(SimCost cost, BBCC* bbcc, InstrInfo* ii, ULong exe_count)
{
   if (!CLG_(clo).simulate_cache)
      cost[fullOffset(EG_IR)] += exe_count;

   if (ii->eventset)
      CLG_(add_and_zero_cost2)(CLG_(sets).full, cost, ii->eventset,
