...
typedef struct {
   ...
   ULong mL; /* misses in the last (third) level cache */
} CacheCC;
...
static void get_debug_info(Addr          instr_addr,
                           const HChar** dir,
                           const HChar** file,
                           const HChar** fn,
                           UInt*         line)
                                                     ...
                                                     InstrInfo* n2)
{
   ...
   cachesim_I1_doref_NoX(n2->instr_addr, n2->instr_len, &n2->parent->Ir.m1,
                         &n2->parent->Ir.m2, &n2->parent->Ir.mL);
   n2->parent->Ir.a++;
}
                                                     ...
                                                     InstrInfo* n3)
{
   ...
   cachesim_I1_doref_NoX(n3->instr_addr, n3->instr_len, &n3->parent->Ir.m1,
                         &n3->parent->Ir.m2, &n3->parent->Ir.mL);
   n3->parent->Ir.a++;
}
                                                      ...
                                                      Word       data_size)
{
   ...
   cachesim_D1_doref(data_addr, data_size, &n->parent->Dr.m1, &n->parent->Dr.m2,
                     &n->parent->Dr.mL);
   n->parent->Dr.a++;
}
                                                      ...
                                                      Word       data_size)
{
   ...
   cachesim_D1_doref(data_addr, data_size, &n->parent->Dw.m1, &n->parent->Dw.m2,
                     &n->parent->Dw.mL);
   n->parent->Dw.a++;
}
...
static VG_REGPARM(3) void log_0Ir_1Dr_cache_access(InstrInfo* n,
                                                   Addr       data_addr,
                                                   Word       data_size)
{
...
static VG_REGPARM(3) void log_0Ir_1Dw_cache_access(InstrInfo* n,
                                                   Addr       data_addr,
                                                   Word       data_size)
{
...
static void
addEvent_Dr(CgState* cgs, InstrInfo* inode, Int datasize, IRAtom* ea)
{
...
static void
addEvent_Dw(CgState* cgs, InstrInfo* inode, Int datasize, IRAtom* ea)
{
   tl_assert(isIRAtom(ea));
...
static void addEvent_D_guarded(CgState*   cgs,
                               InstrInfo* inode,
                               Int        datasize,
                               IRAtom*    ea,
                               IRAtom*    guard,
...
static IRSB* cg_instrument(VgCallbackClosure*     closure,
                           IRSB*                  sbIn,
                           ...
                           IRType                 hWordTy)
{
   ...
   for (/*use current i*/; i < sbIn->stmts_used; i++) {
      ...
      case Ist_StoreG: {
         ...
         addEvent_D_guarded(&cgs, curr_inode, sizeofIRType(type), addr,
      ...
      case Ist_LLSC: {
         ...
         if (st->Ist.LLSC.storedata == NULL) {
            ...
            addEvent_Dr(&cgs, curr_inode, sizeofIRType(dataTy),
                        st->Ist.LLSC.addr);
            ...
            addEvent_Dw(&cgs, curr_inode, sizeofIRType(dataTy),
                        st->Ist.LLSC.addr);
         }
         break;
      }
...
static void fprint_CC_table_and_calc_totals(void)
{
   ...
   fp =
      VG_(fopen)(cachegrind_out_file, VKI_O_CREAT | VKI_O_TRUNC | VKI_O_WRONLY,
   ...
   if (clo_cache_sim) {
      // "desc:" lines (giving I1/D1/L2/LL cache configuration). The spaces
      // after the 2nd colon makes cg_annotate's output look nicer.
      VG_(fprintf)(fp,
                   "desc: I1 cache:         %s\n"
                   "desc: D1 cache:         %s\n"
                   "desc: L2 cache:         %s\n"
   ...
   while ((lineCC = VG_(OSetGen_Next)(CC_table))) {
      ...
      if (clo_cache_sim && clo_branch_sim) {
         VG_(fprintf)(fp,
                      ...
                      " %llu %llu %llu %llu"
                      " %llu %llu %llu %llu\n",
                      lineCC->loc.line, lineCC->Ir.a, lineCC->Ir.m1,
                      lineCC->Ir.m2, lineCC->Ir.mL, lineCC->Dr.a, lineCC->Dr.m1,
                      lineCC->Dr.m2, lineCC->Dr.mL, lineCC->Dw.a, lineCC->Dw.m1,
                      lineCC->Dw.m2, lineCC->Dw.mL, lineCC->Bc.b, lineCC->Bc.mp,
         ...
         VG_(fprintf)(fp,
                      ...
                      lineCC->Ir.m2, lineCC->Ir.mL, lineCC->Dr.a, lineCC->Dr.m1,
                      lineCC->Dr.m2, lineCC->Dr.mL, lineCC->Dw.a, lineCC->Dw.m1,
                      lineCC->Dw.m2, lineCC->Dw.mL);
      } else if (!clo_cache_sim && clo_branch_sim) {
         VG_(fprintf)(fp,
                      "%d %llu"
                      " %llu %llu %llu %llu\n",
                      lineCC->loc.line, lineCC->Ir.a, lineCC->Bc.b,
                      lineCC->Bc.mp, lineCC->Bi.b, lineCC->Bi.mp);
      } else {
   ...
   if (clo_cache_sim && clo_branch_sim) {
      VG_(fprintf)(fp,
                   ...
                   Bc_total.b, Bc_total.mp, Bi_total.b, Bi_total.mp);
   } else if (clo_cache_sim && !clo_branch_sim) {
      VG_(fprintf)(fp,
                   "summary:"
                   " %llu %llu %llu %llu"
      ...
      VG_(fprintf)(fp,
                   ...
                   " %llu"
                   " %llu %llu %llu %llu\n",
                   Ir_total.a, Bc_total.b, Bc_total.mp, Bi_total.b,
                   Bi_total.mp);
   } else {
      VG_(fprintf)(fp,
                   "summary:"
...
#define CG_MAX(a, b) ((a) >= (b) ? (a) : (b))
   ...
   if (clo_cache_sim) {
      ...
      VG_(umsg)("L1_hit: %dllu", D_total.a - D_total.m1);
      VG_(umsg)("L2_hit: %dllu", D_total.m1 - D_total.m2);
      VG_(umsg)("L3_hit: %dllu", D_total.m2 - D_total.mL);
      VG_(umsg)("L3_miss: %dllu", D_total.mL);
      ...
      VG_(umsg)("D1  miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
                D_total.m1 * 100.0 / D_total.a, l2,
                Dr_total.m1 * 100.0 / Dr_total.a, l3,
                Dw_total.m1 * 100.0 / Dw_total.a);
      VG_(umsg)("L2d  miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
                D_total.m2 * 100.0 / D_total.a, l2,
                Dr_total.m2 * 100.0 / Dr_total.a, l3,
                Dw_total.m2 * 100.0 / Dw_total.a);
      VG_(umsg)("LLd miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
                D_total.mL * 100.0 / D_total.a, l2,
                Dr_total.mL * 100.0 / Dr_total.a, l3,
                Dw_total.mL * 100.0 / Dw_total.a);
      VG_(umsg)("\n");

      /* LL overall results */

      LL_total = Dr_total.m1 + Dw_total.m1 + Ir_total.m1 + Dr_total.m2 +
                 Dw_total.m2 + Ir_total.m2;
      LL_total_r = Dr_total.m1 + Ir_total.m1 + Dr_total.m2 + Ir_total.m2;
      LL_total_w = Dw_total.m1 + Dw_total.m2;
   ...
   if (clo_branch_sim) {
      ...
      VG_(umsg)("Mispred rate:  %*.1f%% (%*.1f%%     + %*.1f%%   )\n", l1,
                B_total.mp * 100.0 / B_total.b, l2,
                Bc_total.mp * 100.0 / Bc_total.b, l3,
   ...
   if (VG_(clo_stats)) {
      Int debug_lookups =
         full_debugs + fn_debugs + file_line_debugs + no_debugs;
...
{
   if (VG_(str_clo_cache_opt)(arg, &clo_I1_cache, &clo_D1_cache, &clo_L2_cache,
                              &clo_LL_cache)) {
   }
...
static void set_instr_enabled(Bool enable)
{
   if (enable) {
      ...
      if (!instr_enabled) {
         ...
         VG_(discard_translations_safely)((Addr)0x1000, ~(SizeT)0xfff,
                                          "cachegrind");
         instr_enabled = True;
      } else {
...
static void cg_pre_clo_init(void)
{
   ...
   VG_(needs_command_line_options)(cg_process_cmd_line_option, cg_print_usage,
...
static void cg_post_clo_init(void)
{
   ...
   if (clo_cache_sim) {
      VG_(post_clo_init_configure_caches)(&I1c, &D1c, &L2c, &LLc, &clo_I1_cache,
                                          &clo_D1_cache, &clo_L2_cache,
                                          &clo_LL_cache);
      ...
      min_line_size =
         (L2c.line_size < min_line_size) ? L2c.line_size : min_line_size;
      min_line_size =
      ...
      if (min_line_size < largest_load_or_store_size) {
         ...
         VG_(umsg)("Cachegrind: cannot continue: the minimum line size (%d)\n",
                   (Int)min_line_size);
         VG_(umsg)("  must be equal to or larger than the maximum register "
                   "size (%d)\n",
      ...
      VG_(umsg)("Initialized caches with size (order I1, D1, L2, LLC): "
                "%d, %d, %d, %d\n",
                I1c.size, D1c.size, L2c.size, LLc.size);
   }
